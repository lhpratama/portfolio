...
#Inserting data of top 7 film by IMDB Rating
dftopimdb = df.sort_values('IMDB Rating', ascending = False).head(7)
dftopimdb = dftopimdb.set_index('Title')  # Setting index

#Inserting data of top 7 film by Metascore
dftopmetascore = df.sort_values('Metascore', ascending = False).head(7)
dftopmetascore = dftopmetascore.set_index('Title')  # Setting index

#Inserting data of top 7 film by Votes
dftopvotes = df.sort_values('Votes', ascending = False).head(7)
dftopvotes = dftopvotes.set_index('Title') # Setting index
...
def index():
    ...
    card_data = f'{dftopimdb["IMDB Rating"].mean().round(2)}'  # Average IMDB Rating
    card_data1 = f'{dftopvotes["Votes"].mean().round(2)}'  # Average Votes 
    card_data2 = f'{dftopmetascore["Metascore"].mean().round(2)}'  # Average Metascore

    # Generate plot for By IMDB Rating
    fig_imdb, ax_imdb = plt.subplots(figsize=(14, 4))  # Adjust the width (14) and height (4) as needed
    dftopimdb['IMDB Rating'].plot(kind='barh', ax=ax_imdb).invert_yaxis()
    ...
    fig_votes, ax_votes = plt.subplots(figsize=(14, 4))  # Adjust the width (14) and height (4) as needed
    dftopvotes['Votes'].plot(kind='barh', ax=ax_votes).invert_yaxis()
    ...
    fig_metascore, ax_metascore = plt.subplots(figsize=(14, 4))  # Adjust the width (14) and height (4) as needed
    dftopmetascore['Metascore'].plot(kind='barh', ax=ax_metascore).invert_yaxis()
